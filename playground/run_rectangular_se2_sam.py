...
def main():
    # データ作成
    t_mat_sim = rectangular_data(good_data=True)
    t_mat_input = rectangular_data(good_data=False)
    number = len(t_mat_sim)
    ...
    # Landmarks
    landmarks_sim = np.array([[0.75, 0.5]])  # a landmark at origin
    ...
    for _t_mat in t_mat_input:
        tmp_t_mat.append(noised_transformation_matrix(_t_mat, 3. * u_sigma))
